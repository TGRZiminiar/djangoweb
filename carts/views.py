...
def add_cart(request,product_id):
    ...
    if current_user.is_authenticated: #ถ้า user login
        product_variation = []
        ...
        is_cart_item_exists = CartItem.objects.filter(product = product , user=current_user).exists() #กรองว่ามีข้อมูลที่เป็นสินค้าอยู่ในcartไหม
        if is_cart_item_exists: #ถ้ามีสินค้าอยู่ในcart
            cart_item = CartItem.objects.filter(product=product, user=current_user) #return cartitem object
        ...
        is_cart_item_exists = CartItem.objects.filter(product = product , cart=cart).exists() #กรองว่ามีข้อมูลที่เป็นสินค้าอยู่ในcartไหม
        if is_cart_item_exists: #ถ้ามีสินค้าอยู่ในcart
...
@login_required(login_url='login') #ต้องล็อคอินเท่านั่น
